module.exports = {
    ...
    selectTodaySchedules: "SELECT s.id, s.calendarId, s.title, s.category, s.dueDateClass, s.start, s.end FROM baeuda.schedule as s JOIN baeuda.calendar as c ON s.calendarId=c.id WHERE c.user_id=? AND (DAY(start)=DAY(now()) AND DAY(end)=DAY(now()))"
};